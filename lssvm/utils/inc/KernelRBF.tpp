...
/**
 * @brief  Radial Bases Function (RBF) Kernel implementation.
 *
 * The class implements the RBF kernel function in the form of  
 * \f[ 
      \exp\left[  - \frac{ ||\mathbf{x}_i - \mathbf{x}_j ||^2 }{\gamma}   \right] 
      = \exp\left[ -\frac{\sum_{k=1}^{d} (x_k^i-x_k^j)(x_k^i-x_k^j)}{\gamma}  \right]
   \f]
 ...
 *     either as a scalar or vector (the corresponding KernelRBF::SetBandWidth 
 *     method will be invoked).
 *  - KernelBase::Evaluate will invoke the KernelRBF::EvaluateKernel method to evaluate
 *     the kernel function (the KernelRBF:EvaluateKernelRBF method will be invoked)
 *
  \rst
...
//        also defines the type of the RBF kernel parameter i.e. the bandwidth. 
template <typename T, typename TInpD>
class KernelRBF : public KernelBase < KernelRBF <T, TInpD>, T > {
public:
 ...
 ~KernelRBF() { }
  ...
  /**@brief The concrete implementation for setting the **scalar** kernel bandwidth. 
    *@param[in] bw  the scalar RBF kernel bandwidth \f$\gamma\f$
    */
  ...
  /**@brief The concrete implementation for setting the **vector** kernel bandwidth. 
    *@param[in] bw  the RBF kernel bandwidth vector \f$\boldsymbol{\gamma}\f$
    */
  ...
  /**
    ...
    * method by passing the argument(s). The appropriate EvaluateKernelRBF method
  ...
  /**
    ...
    */
  T EvaluateKernelRBF(const TInpD* a, const TInpD* b, std::size_t num) {
    ...
    return (fDim==1) ? EvaluateKernelRBF1D(a,b,num) : EvaluateKernelRBFMultiD(a,b,num);
  }
...
private:
  ...
  T EvaluateKernelRBF1D(const TInpD* a, const TInpD* b, std::size_t num) {
      T dum = 0.0;
//#pragma clang loop vectorize(enable)
      ...
      return std::exp(dum*fInvBandWV[0]);
  }
  /**@brief Evaluation of the kernel function when vector bandwidth is used. */
  T EvaluateKernelRBFMultiD(const TInpD* a, const TInpD* b, std::size_t num) {
      T dum = 0.0;
//#pragma clang loop vectorize(enable)
      for (std::size_t id=0; id<num; ++id) {
        const T dum1 = a[id] - b[id];
        dum -= dum1*dum1*fInvBandWV[id];
      }
      return std::exp(dum);
  }
 ...
 */
// @{  
  /**@brief Dimension of the RBF kernel bandwidth parameter. */
  std::size_t    fDim;
  /**@brief The RBF kernel bandwidth parameter value(s). */
